...
CUPONS_DESCONTO = {
    ...
    "INAUGURA15": {"desconto_pct": 15, "descricao": "15% inauguracao", "validade": "2026-08-31"},
}
...
VEICULOS_INFO = {
    "Moto": {"peso": "20 kg", "volume": "0,1 m3", "pallets": "", "desc": "Documentos e pequenos volumes"},
    "Carro": {"peso": "300 kg", "volume": "1 m3", "pallets": "", "desc": "Urgencias e encomendas leves"},
    "Fiorino": {"peso": "600 kg", "volume": "3,5 m3", "pallets": "", "desc": "Entregas urbanas leves"},
    "Van": {"peso": "1.500 kg", "volume": "12 m3", "pallets": "6", "desc": "E-commerce e cargas medias"},
    "VUC": {"peso": "2.000 kg", "volume": "18 m3", "pallets": "8", "desc": "Carga urbana (entra na ZMRC)"},
    "3/4": {"peso": "3.500 kg", "volume": "25 m3", "pallets": "10", "desc": "Distribuicoes regionais"},
    "Toco": {"peso": "7.000 kg", "volume": "35 m3", "pallets": "12-14", "desc": "Eixo simples, carga media-pesada"},
    "Truck": {"peso": "14.000 kg", "volume": "45 m3", "pallets": "16-18", "desc": "Eixo duplo, grandes volumes"},
    "Carreta": {"peso": "27.000 kg", "volume": "90 m3", "pallets": "24-28", "desc": "Carga completa pesada"},
}

ADICIONAIS = {
    "Horario comercial": 0,
    "Horario de pico": 0.15,
    "Periodo noturno": 0.25,
    "Sabado": 0.20,
...
if aba == "Cotacao de Frete":
    ...
    if codigo_cliente and tier_ativo is None:
        st.error("Codigo invalido. Verifique com seu consultor.")
    elif tier_ativo:
    ...
    if tier_ativo:
        ...
        if cupom_input:
            ...
            if cupom_dados:
                ...
                st.caption("Cupom invalido ou expirado.")

        st.markdown('<div class="section-label">Veiculo</div>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            if tipo_carga == "Complemento":
                percentual_complemento = st.slider("Espaco utilizado (%)", 40, 100, 60)
            else:
        ...
        with col_vol:
            volume_carga = st.number_input("Volume (m3)", min_value=0.1, value=2.0, step=0.5)
        ...
        with col_km:
            km_ida = st.number_input("Distancia ida (km)", min_value=1.0, max_value=1000.0, value=25.0, step=1.0)
        with col_paradas:
        ...
        st.markdown('<span class="locked-badge">Roteirizador automatico disponivel em breve</span>', unsafe_allow_html=True)

        st.markdown('<div class="section-label">Periodo</div>', unsafe_allow_html=True)
        adicional = st.selectbox("Periodo", list(ADICIONAIS.keys()), label_visibility="collapsed")

        st.markdown('<div class="section-label">Protecao de carga</div>', unsafe_allow_html=True)
        ...
        if protecao_ativa and valor_mercadoria > 0:
            ...
            st.caption(f"Protecao: {formatar_brl(vp)}")

        st.markdown('<div class="info-box">Pedagio, taxas de acesso e estadias nao inclusos. Cobrados a parte conforme percurso.</div>', unsafe_allow_html=True)
        ...
        if calcular:
            ...
            # RESULTADO PRINCIPAL
            st.markdown(f'<div class="result-card"><div class="result-label">VALOR DA COTACAO</div><div class="result-total">{formatar_brl(resultado["total"])}</div></div>', unsafe_allow_html=True)

            # KPIs
            st.markdown(f'<div class="kpi-row"><div class="kpi-item"><div class="kpi-item-label">KM cobrado</div><div class="kpi-item-value">{resultado["km_cobrado"]:.0f} km</div></div><div class="kpi-item"><div class="kpi-item-label">Paradas</div><div class="kpi-item-value">{n_paradas}</div></div><div class="kpi-item"><div class="kpi-item-label">Veiculo</div><div class="kpi-item-value">{veiculo_selecionado}</div></div></div>', unsafe_allow_html=True)

            # DETALHAMENTO - linha por linha para evitar bug
            ...
            if resultado['protecao'] > 0:
                st.markdown(f'<div class="breakdown-row"><span class="breakdown-name">Protecao de carga</span><span class="breakdown-value">{formatar_brl(resultado["protecao"])}</span></div>', unsafe_allow_html=True)
            ...
            # BOTAO WHATSAPP
            msg_whatsapp = f"Ola! Gostaria de solicitar um frete.\n\n*Cotacao Transfelog App*\nVeiculo: {veiculo_selecionado}\nDistancia: {km_ida} km (cobrado: {resultado['km_cobrado']:.0f} km)\nPontos de entrega: {n_paradas}\nPeriodo: {adicional}\nTipo: {tipo_carga}\n{'Protecao de carga: Inclusa' if protecao_ativa else 'Protecao: Nao inclusa'}\n\n*TOTAL: {formatar_brl(resultado['total'])}*\nPedagio e taxas cobrados a parte.\n\nGostaria de confirmar esta solicitacao."
            ...
            # TEXTO PARA COPIAR
            st.markdown("")
            st.caption("Ou copie a cotacao:")
            msg_copiar = f"*TRANSFELOG | Cotacao de Frete*\n\nVeiculo: {veiculo_selecionado}\nDistancia: {km_ida} km (cobrado: {resultado['km_cobrado']:.0f} km)\nPontos de entrega: {n_paradas}\nPeriodo: {adicional}\n{'Protecao de carga: Inclusa' if protecao_ativa else ''}\n{'Cupom: ' + cupom_dados['descricao'] if cupom_dados else ''}\n\n*TOTAL: {formatar_brl(resultado['total'])}*\nPedagio e taxas cobrados a parte.\n\ntransfelog.streamlit.app"

            st.text_area("Copiar", msg_copiar, height=200, label_visibility="collapsed")

    else:
        if not codigo_cliente:
            st.caption("Insira seu codigo de acesso para calcular sua cotacao.")


elif aba == "Cadastro de Motorista":

    st.markdown('<div class="section-label">Cadastro para aprovacao</div>', unsafe_allow_html=True)
    st.caption("Preencha todos os campos e envie seus documentos. Apos analise, voce recebera seu acesso em ate 48 horas.")

    st.markdown("")

    nome_completo = st.text_input("Nome completo")
    endereco = st.text_input("Endereco completo (rua, numero, bairro, cidade, estado)")
    ...
    with col_cap2:
        capacidade_volume = st.number_input("Capacidade de volume (m3)", min_value=1.0, value=18.0, step=1.0)
    ...
    valor_km_desejado = st.number_input(
        ...
        value=5.00,
        step=0.50,
        help="Sujeito a aprovacao conforme tabela vigente."
    )
    ...
    horario = st.selectbox(
        ...
        ["Integral (08h as 20h)", "Manha (06h as 14h)", "Tarde (12h as 20h)", "Noturno (18h as 06h)", "Flexivel"]
    )

    st.markdown('<div class="section-label">Documentos</div>', unsafe_allow_html=True)
    st.caption("Envie fotos legiveis. Formatos: PNG, JPG ou PDF.")

    cnh_upload = st.file_uploader("CNH (frente e verso)", type=["png", "jpg", "jpeg", "pdf"], key="cnh")
    doc_veiculo_upload = st.file_uploader("Documento do veiculo (CRLV)", type=["png", "jpg", "jpeg", "pdf"], key="doc_veic")
    foto_veiculo = st.file_uploader("Foto do veiculo (opcional)", type=["png", "jpg", "jpeg"], key="foto_veic")

    st.markdown("")
    aceite_termos = st.checkbox("Declaro que as informacoes sao verdadeiras e aceito os termos de parceria.")
    ...
    if enviar:
        ...
        if not all(campos_obrigatorios):
            st.error("Preencha todos os campos obrigatorios e anexe CNH + CRLV.")
            ...
            st.markdown(f'<div class="success-box">Cadastro enviado com sucesso! Documentos em analise.<br>Retorno em ate 48h no WhatsApp: <b>{whatsapp_motorista}</b></div>', unsafe_allow_html=True)
...
| Placa | {placa} |
| Capacidade | {capacidade_peso} kg / {capacidade_volume} m3 |
