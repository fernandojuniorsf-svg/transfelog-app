...
def get_drive_service():
    if not GDRIVE_DISPONIVEL:
        st.warning("Biblioteca google-api-python-client nao instalada. Adicione ao requirements.txt")
        return None
    try:
        credentials = get_credentials()
        ...
        st.error(f"Erro ao conectar Google Drive: {str(e)}")
        return None
...
CUPONS_DESCONTO = {
    ...
    "INAUGURA15": {"desconto_pct": 15, "descricao": "15% de inauguracao", "validade": "2026-08-31"},
}
...
VEICULOS_INFO = {
    ...
    "Carro": {"peso": "300 kg", "volume": "1 m\u00b3", "pallets": "", "desc": "Urgencias e encomendas leves"},
    "Fiorino": {"peso": "600 kg", "volume": "3,5 m\u00b3", "pallets": "", "desc": "Entregas urbanas leves"},
    "Van": {"peso": "1.500 kg", "volume": "12 m\u00b3", "pallets": "6", "desc": "E-commerce e cargas medias"},
    "VUC": {"peso": "2.000 kg", "volume": "18 m\u00b3", "pallets": "8", "desc": "Carga urbana (entra na ZMRC)"},
    "3/4": {"peso": "3.500 kg", "volume": "25 m\u00b3", "pallets": "10", "desc": "Distribuicoes regionais"},
    "Toco": {"peso": "7.000 kg", "volume": "35 m\u00b3", "pallets": "12-14", "desc": "Eixo simples, carga media-pesada"},
...
if aba == "Cotacao de Frete":
    ...
    if tier_ativo:
        ...
        if calcular:
            ...
            dados_cotacao = [
                ...
                str(n_paradas),
                adicional,
                "Sim" if protecao_ativa else "Nao",
    ...
    st.markdown('<div class="section-label">Endereco</div>', unsafe_allow_html=True)
    cep_motorista = st.text_input("CEP", placeholder="Ex: 09720-000")
    endereco = st.text_input("Rua, numero e bairro")
    ...
    if enviar:
        ...
        if not all(campos_obrigatorios):
            ...
            if sucesso_sheets:
                st.markdown(f'<div class="success-box">Cadastro enviado com sucesso! Documentos salvos.<br>Retorno em ate 48h no WhatsApp: <b>{whatsapp_motorista}</b></div>', unsafe_allow_html=True)
            else:
...
| CEP | {cep_motorista} |
| Endereco | {endereco}, {cidade_motorista} - {estado_motorista} |
...
| Foto veiculo | {'Enviada' if link_foto else 'Nao enviada'} |
"""
            st.markdown(resumo)
